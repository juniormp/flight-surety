...
contract FlightSuretyData is IFlightSuretyData {
    ...
    modifier requireUniqueAirline(address _airline) {
        require(airlines[_airline].registered == false, "An airline can only be registered once.");
        _;
    }

    modifier requireUniqueFlight(address _airline, string memory _flight, uint256 _timestamp) {
        bytes32 flightKey = getFlightKey(_airline, _flight, _timestamp);

        require(flights[flightKey].registered == false, "A flight can only be registered once.");
        _;
    }
     ...
     *
     */
    function registerAirline(address airline) public requireIsOperational requireIsCallerAuthorized requireUniqueAirline(airline) returns (address)
    {
        ...
        airlines[airline].id = numberOfAirlines;
        airlines[airline].account = airline;
    ...
    function getNumberOfAirlines() external view requireIsOperational requireIsCallerAuthorized returns (uint256)
    {
        return numberOfAirlines;
    }

    function getAirline(address airline) external view requireIsOperational requireIsCallerAuthorized returns (uint256 id, address account, bool registered, bool funded)
    {
        id = airlines[airline].id;
        account = airlines[airline].account;
        registered = airlines[airline].registered;
        funded = airlines[airline].funded;

        return (id, account, registered, funded);
    }

    function updateAirlineToFunded(address airline) public payable requireIsOperational requireIsCallerAuthorized
    {
        // If the airline hasn't previously provided funding then increment the funded tally.
        if (airlines[airline].funded == false) {
    ...
    function getNumberOfFundedAirlines() external view requireIsOperational requireIsCallerAuthorized returns (uint256)
    {
    ...
    function registerFlight(address _airline, string calldata _flight, uint256 _timestamp) external requireIsOperational requireIsCallerAuthorized requireUniqueFlight(_airline, _flight, _timestamp) returns (bytes32) {
        bytes32 flightKey = getFlightKey(_airline, _flight, _timestamp);
    ...
    function getNumberOfFlights() external view requireIsOperational requireIsCallerAuthorized returns (uint256) {
        return numberOfFlights;
    }

    function getFlight(address _airline, string calldata _flight, uint256 _timestamp) external view requireIsOperational requireIsCallerAuthorized returns (uint256 id, string memory code, bool registered, uint8 statusCode, uint256 timestamp, address airline) {
        bytes32 flightKey = getFlightKey(_airline, _flight, _timestamp);
     ...
     *
     */
    function buy(address _airline, string calldata _flight, uint256 _timestamp, address _passenger, uint256 _amount) external payable requireIsOperational requireIsCallerAuthorized {
        bytes32 flightKey = getFlightKey(_airline, _flight, _timestamp);
    ...
    function getInsurance(address _airline, string calldata _flight, uint256 _timestamp, address _passenger) external view
        returns (
            bool insured,
            uint256 insuredFor,
            uint256 credit
     ...
     */
    function creditInsurees(address _airline, string calldata _flight, uint256 _timestamp, uint8 _statusCode, uint8 _factor) external requireIsOperational requireIsCallerAuthorized {
        bytes32 flightKey = getFlightKey(_airline, _flight, _timestamp);
     ...
     *
     */
    function pay(address payable _passenger) external requireIsOperational requireIsCallerAuthorized
    {
    ...
    function getCredit(address _passenger) external view returns (uint256 credit)
    {
        if (passengersWithCredit[_passenger].account == _passenger) {
