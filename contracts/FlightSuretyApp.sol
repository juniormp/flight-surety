...
contract FlightSuretyApp {
    ...
    modifier requireRequestFromRegisteredAirline() {
        require(_isAirlineRegistered(msg.sender), "Caller is not a registered airline.");
        _;
    }

    modifier requireRequestFromFundedAirline() {
        require(_isAirlineFunded(msg.sender), "Calling airline is not funded.");
        _;
    }

    modifier requireNewAirline(address _airline) {
        require(
            !_isAirlineRegistered(_airline),
    ...
    modifier requireNewFlight(
        ...
        require(!_isFlightRegistered(_airline, _flight, _timestamp), "The flight has already been registered.");
        _;
    }
    ...
    modifier requireRegisteredFlight(
        ...
        require(
            _isFlightRegistered(_airline, _flight, _timestamp),
    ...
    function _isAirlineRegistered(address airline) internal view returns (bool)
    {
        bool airlineRegistered = false;
        (, , airlineRegistered, ) = flightSuretyData.getAirline(airline);

        return airlineRegistered;
    }

    function _isAirlineFunded(address _airline) internal view returns (bool) {
    ...
    function _registerFoundingAirline(address _airline) internal returns (bool success, uint256 votes)
    {
    ...
    function _registerByConsensusAirline( address _airline, uint256 _numberOfFundedAirlines)  internal requireUniqueVote(_airline) returns (bool success, uint256 votes)
    {
        Votes storage voters = multiPartyConsensusVotes[_airline];
     ...
     *
     */
    function registerAirline(address _airline) external requireRequestFromFundedAirline requireNewAirline(_airline) returns (bool success, uint256 votes)
    {
        ...
        if (numberOfFundedAirlines < M) {
            ...
            return
                _registerByConsensusAirline(_airline, numberOfFundedAirlines);
        }
    }

    function fundAirline() external payable requireSufficientFunds requireRequestFromRegisteredAirline {
     ...
     *
     */
    function registerFlight(address airline, string calldata flight, uint256 timestamp) external requireRequestFromRegisteredAirline requireNewFlight(airline, flight, timestamp)
    {
        flightSuretyData.registerFlight(airline, flight, timestamp);
        emit FlightRegistered(airline, flight, timestamp);
    }

    function _isFlightRegistered( address airline, string memory flight, uint256 timestamp) internal view returns (bool) {
        bool registeredFlight = false;
        (, , registeredFlight, , , ) = flightSuretyData.getFlight(airline, flight, timestamp);
